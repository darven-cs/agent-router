...
```bash
PHASE_INFO=$(node "/home/darven/桌面/dev_app/agent-router/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "/home/darven/桌面/dev_app/agent-router/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "/home/darven/桌面/dev_app/agent-router/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
